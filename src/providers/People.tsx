...
export default function PeopleProvider({children}:{children: ReactNode}) {
 ...
 const pushPerson = ({name, CPF, birthday}: Omit<PersonType, "id">) => {
  setPeople((oldPeople) => [...oldPeople, {name, CPF, birthday, id: `${Math.random()}`}])
  saveAsyncData([...people, {name, CPF, birthday, id: `${Math.random()}`}])
 }

 const updatePerson = ({id, name, CPF, birthday}: PersonType) => {
  setPeople((oldPeople) => [...oldPeople.filter(person => person.id !== id), {name, CPF, birthday, id}])
  saveAsyncData([...people.filter(person => person.id !== id), {name, CPF, birthday, id}])
 }
...
const saveAsyncData = async (p: PersonType[]) => {
 ...
 return (
   <PeopleContext.Provider value={{people, pushPerson, updatePerson, removePerson, loadAsyncData}}>
    {children}
